...
def label2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 0, axis=0)] = [0, 255, 0]
...
def kappa_score(y_hat, y, ignore_index=None):
    ...
    pred = np.argmax(y_hat, axis=1).ravel()  # shape (B*H*W,)
    true = np.argmax(y, axis=1).ravel()      # shape (B*H*W,)
...
def main():
    ...
    with torch.no_grad():
        ...
        for input in tqdm(test_loader):
            ...
            kappa = kappa_score(raw_predictions, masks_true)  # 返回一个标量
